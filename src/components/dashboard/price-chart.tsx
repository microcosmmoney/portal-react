  ...
  return (
    <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl overflow-hidden blockchain-card">
      <div className="flex flex-col sm:flex-row items-stretch border-b border-white/10">
        <div className="flex flex-1 flex-col justify-center gap-1 px-2 py-2 2xs:px-3 sm:px-6 sm:py-4">
          <div className="text-white font-mono font-bold text-xs 2xs:text-sm sm:text-base">
            {t('mccPriceTrend', 'MCC Price Trend')}
          </div>
          <div className="text-neutral-500 font-mono text-[10px] 2xs:text-xs">
          ...
          {TIME_RANGES.map((tr) => {
            ...
            return (
              <button
                ...
                className={`relative z-10 flex flex-1 flex-col justify-center gap-0.5 2xs:gap-1 border-t border-white/10 px-2 py-2 2xs:px-3 2xs:py-3 text-left even:border-l even:border-white/10 sm:border-t-0 sm:border-l sm:border-white/10 sm:px-5 sm:py-4 transition-colors ${
                  isActive ? 'bg-white/10' : 'hover:bg-white/5'
                }`}
                onClick={() => setRange(tr.value)}
              >
                <span className="text-[9px] 2xs:text-[10px] text-neutral-500 font-mono tracking-wider">
                  {tr.label}
                </span>
                {change == null ? (
                  <span className="text-sm 2xs:text-base xs:text-lg font-bold leading-none text-neutral-600 font-mono sm:text-xl">...</span>
                ) : (
                  <span className={`text-sm 2xs:text-base xs:text-lg font-bold leading-none font-mono sm:text-xl tabular-nums ${isPositive ? 'text-cyan-400' : 'text-red-400'}`}>
      ...
      <div className="p-2 2xs:p-3 sm:p-6">
        <div className="h-[160px] 2xs:h-[200px] xs:h-[260px] sm:h-[360px]">
          {loading ? (
