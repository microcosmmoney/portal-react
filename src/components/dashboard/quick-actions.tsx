...
export function MicrocosmQuickActions({ basePath = '', onNavigate, accentColor }: MicrocosmQuickActionsProps) {
  ...
  return (
    <div className="mb-2 2xs:mb-3 sm:mb-6">
      <div className="grid grid-cols-2 2xs:grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-1.5 2xs:gap-2 sm:gap-3">
        {exchanges.map((ex) => {
          const Icon = ex.icon
          ...
          return (
            ...
            >
              <div
                className={`backdrop-blur-md bg-white/5 border ${ex.color} rounded-xl p-2 2xs:p-2.5 sm:p-4 flex items-center gap-1.5 2xs:gap-2 sm:gap-3 cursor-pointer active:scale-[0.98] blockchain-card min-w-0`}
                style={ex.colorStyle}
              >
                <Icon stroke={iconStroke} />
                <span className="font-mono text-[11px] 2xs:text-xs sm:text-sm font-bold tracking-wider truncate">
                  {ex.label}
                </span>
              </div>
            </a>
