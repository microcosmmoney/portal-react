...
export function MicrocosmDashboardOverview({
  ...
  return (
    <div className="max-w-7xl mx-auto font-mono p-6 space-y-6" style={rootStyle}>
      {/* === PRIORITY 1: Instant render === */}
      {showHeader && (
      ...
      {/* PriceChart: synchronous import to avoid lazy() issues in npm packages */}
      <SafeRender><MicrocosmPriceChart accentColor={accentColor} /></SafeRender>

      {/* === PRIORITY 2: Below-fold — lazy via IntersectionObserver === */}
      <LazySection fallback={
        <div className="grid lg:grid-cols-2 gap-6"><CardSkeleton height="h-56" /><CardSkeleton height="h-56" /></div>
      }>
      ...
      <LazySection fallback={
        <div className="grid lg:grid-cols-2 gap-6"><CardSkeleton /><CardSkeleton /></div>
      }>
        <div className="grid lg:grid-cols-2 gap-6">
