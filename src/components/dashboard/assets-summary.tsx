...
export function MicrocosmAssetsSummary({ basePath = '', accentColor }: MicrocosmAssetsSummaryProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1.5 2xs:gap-2 sm:gap-3">
      {}
      <Link href={resolvePath('/mcc/wallet')} className="block group">
        <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-2 2xs:p-3 sm:p-4 blockchain-card h-full">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[#5EEAD4] text-[8px] 2xs:text-[9px] sm:text-[10px] font-mono tracking-widest uppercase truncate">{t('mccBalance', 'MCC_BALANCE')}</span>
          </div>
          <div className={accentColor ? 'text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate' : 'text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate text-cyan-400'} style={accentColor ? { color: accentColor } : undefined}>
            {mccLoading
      ...
      <Link href={resolvePath('/mcc/mcd')} className="block group">
        <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-2 2xs:p-3 sm:p-4 blockchain-card h-full">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[#5EEAD4] text-[8px] 2xs:text-[9px] sm:text-[10px] font-mono tracking-widest uppercase truncate">{t('mcdBalance', 'MCD_BALANCE')}</span>
          </div>
          <div className="text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate text-white">
      ...
      {}
      <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-2 2xs:p-3 sm:p-4 blockchain-card">
        <div className="flex items-center gap-1.5 mb-2">
          <svg className="w-3.5 h-3.5 text-[#5EEAD4]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
          <span className="text-[#5EEAD4] text-[8px] 2xs:text-[9px] sm:text-[10px] font-mono tracking-widest uppercase truncate">{t('locked', 'LOCKED')}</span>
        </div>
        <div className="text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate text-white">
      ...
      <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-2 2xs:p-3 sm:p-4 blockchain-card">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-[#5EEAD4] text-[8px] 2xs:text-[9px] sm:text-[10px] font-mono tracking-widest uppercase truncate">{t('wallets', 'WALLETS')}</span>
        </div>
        <div className="text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate text-white">{walletCount}</div>
      </div>

      {}
      <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-2 2xs:p-3 sm:p-4 blockchain-card">
        <div className="text-[#5EEAD4] text-[8px] 2xs:text-[9px] sm:text-[10px] font-mono tracking-widest uppercase truncate mb-2">{t('rank', 'RANK')}</div>
        <div className={`text-base 2xs:text-lg xs:text-xl sm:text-2xl font-bold font-mono truncate ${rankColor ?? ''}`} style={rankStyle}>
          {userRank || t('na', 'N/A')}
        </div>
        {nextRank && (
          <div className="mt-2">
