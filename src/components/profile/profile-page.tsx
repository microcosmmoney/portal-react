...
export function MicrocosmProfilePage({
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-2 py-3 space-y-2 2xs:px-3 2xs:py-4 2xs:space-y-3 sm:px-6 sm:py-6 sm:space-y-6 font-mono">
        <div>
          <h1 className="text-base 2xs:text-lg xs:text-xl sm:text-2xl md:text-3xl font-bold text-white tracking-wider truncate">{t('title', 'Profile')}</h1>
          <p className="text-[10px] 2xs:text-xs sm:text-sm text-neutral-400">{t('subtitle', 'View and manage your personal information')}</p>
        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 py-3 space-y-2 2xs:px-3 2xs:py-4 2xs:space-y-3 sm:px-6 sm:py-6 sm:space-y-6 font-mono">
      ...
      <TerminalCard>
        <div className="flex flex-col 2xs:flex-row items-start gap-3 2xs:gap-4 mb-4 2xs:mb-6 pb-4 2xs:pb-6 border-b border-neutral-700">
          <div className="relative group shrink-0">
            {profile?.avatar_url ? (
              <img
                src={profile.avatar_url}
                alt=""
                className="h-16 w-16 2xs:h-20 2xs:w-20 rounded bg-neutral-800 border border-neutral-700 object-cover"
              />
            ) : (
              <div className="h-16 w-16 2xs:h-20 2xs:w-20 rounded bg-cyan-400/20 border border-cyan-400/30 flex items-center justify-center text-cyan-400 text-2xl 2xs:text-3xl font-bold">
            ...
            {editing ? (
              ...
              <>
                <div className="flex items-center gap-2 min-w-0">
                  <div className="text-base 2xs:text-lg xs:text-xl text-white truncate">{displayName}</div>
                  <button
                ...
                <div className="text-xs 2xs:text-sm text-neutral-400 mt-1 truncate">{profile?.email || t('noEmail', '(no email)')}</div>
                <div className="flex gap-1.5 2xs:gap-2 mt-2 flex-wrap">
        ...
        <div className="grid grid-cols-1 2xs:grid-cols-2 gap-2 2xs:gap-3 sm:gap-4">
          <div className="bg-white/5 border border-white/10 rounded-lg p-2 2xs:p-3 sm:p-4 blockchain-sub-card min-w-0">
            <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1">UID</div>
            <div className="text-[10px] 2xs:text-xs text-white font-mono break-all">{profile?.uid || userInfo?.uid || '-'}</div>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-lg p-2 2xs:p-3 sm:p-4 blockchain-sub-card min-w-0">
            <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1">{t('shortId', 'Short ID')}</div>
            <div className="text-xs 2xs:text-sm text-white font-mono truncate">{profile?.short_id || '-'}</div>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-lg p-2 2xs:p-3 sm:p-4 blockchain-sub-card min-w-0">
            <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1">{t('created', 'Created')}</div>
            <div className="text-xs 2xs:text-sm text-white truncate">{formatDate(profile?.created_at)}</div>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-lg p-2 2xs:p-3 sm:p-4 blockchain-sub-card min-w-0">
            <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1">{t('lastLogin', 'Last Login')}</div>
            <div className="text-xs 2xs:text-sm text-white truncate">{formatDate(profile?.last_login_at)}</div>
          </div>
        </div>
      </TerminalCard>
      ...
      {levelData && (
        <TerminalCard title={t('levelStatus', 'Level Status')}>
          <div className="space-y-3 2xs:space-y-4">
            <div className="flex items-start justify-between gap-2 p-3 2xs:p-4 bg-neutral-800 rounded">
              <div className="min-w-0 flex-1">
                <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1 truncate">{t('currentLevel', 'CURRENT LEVEL')}</div>
                <div className={`text-base 2xs:text-lg xs:text-xl font-bold truncate ${lvlInfo.color}`}>{lvlInfo.label}</div>
                <div className="text-[10px] 2xs:text-xs text-neutral-500 mt-1 line-clamp-2">{lvlInfo.description}</div>
              </div>
              <div className="text-right shrink-0 min-w-0">
                <div className="text-[10px] 2xs:text-xs text-neutral-400 tracking-wider mb-1 truncate">{t('holdings', 'HOLDINGS')}</div>
                <div className="text-[10px] 2xs:text-xs text-white font-mono">
