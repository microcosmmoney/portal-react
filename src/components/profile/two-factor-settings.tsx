    ...
    <TerminalCard>
      <div className="flex items-center justify-between gap-2 mb-3 2xs:mb-4">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-xs 2xs:text-sm text-neutral-300 font-medium tracking-wider truncate">{t('twoFactorAuth', 'TWO FACTOR AUTH')}</span>
        </div>
        <span
          className={`px-2 py-0.5 rounded text-[10px] 2xs:text-xs whitespace-nowrap shrink-0 ${
      ...
      {step === 'qr' && setupData && (
        <div className="space-y-4">
          ...
          {setupData.qr_code && (
            <div className="flex justify-center bg-white rounded p-3 2xs:p-4">
              <img
                src={setupData.qr_code.startsWith('data:') ? setupData.qr_code : `data:image/png;base64,${setupData.qr_code}`}
                alt="2FA QR Code"
                className="w-40 h-40 2xs:w-48 2xs:h-48"
              />
            </div>
          )}
          <div className="bg-white/5 border border-white/10 rounded-lg p-3 blockchain-sub-card">
