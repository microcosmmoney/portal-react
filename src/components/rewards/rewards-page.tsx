  ...
  return (
    <div className="max-w-7xl mx-auto px-3 py-4 space-y-3 xs:px-4 xs:space-y-4 sm:px-6 sm:py-6 sm:space-y-6 font-mono">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white tracking-wider">Rewards</h1>
          <p className="text-xs sm:text-sm text-neutral-400">Send MCC rewards to your territory members</p>
        </div>
      ...
      {tab === 'history' && (
        <TerminalCard title="Reward History">
          {loading ? (
            <div className="text-center py-8 text-neutral-500">Loading...</div>
          ) : history.length === 0 ? (
            <div className="text-center py-8 text-neutral-500">No records</div>
          ) : (
      ...
      {tab === 'leaderboard' && (
        <div className="grid md:grid-cols-2 gap-6">
          <TerminalCard title="Top Recipients">
            {topRecipients.length === 0 ? (
              <div className="text-center py-4 text-neutral-500">No data</div>
            ) : (
          ...
          <TerminalCard title="Top Senders">
            {topSenders.length === 0 ? (
              <div className="text-center py-4 text-neutral-500">No data</div>
            ) : (
              <div className="space-y-3">
      ...
      {showBatchDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowBatchDialog(false)}>
          <div className="bg-neutral-900 border border-neutral-700 rounded-lg w-full max-w-2xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-white font-medium">Batch Reward Send</h3>
