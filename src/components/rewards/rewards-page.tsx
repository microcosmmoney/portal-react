...
export function MicrocosmRewardsPage({}: MicrocosmRewardsPageProps = {}) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 py-3 space-y-2 2xs:px-3 2xs:py-4 2xs:space-y-3 sm:px-6 sm:py-6 sm:space-y-6 font-mono">
      <div className="flex items-center justify-between flex-wrap gap-2 2xs:gap-3">
        <div className="min-w-0 flex-1">
          <h1 className="text-base 2xs:text-lg xs:text-xl sm:text-2xl md:text-3xl font-bold text-white tracking-wider truncate">{t('title', 'Manager Rewards')}</h1>
          <p className="text-[10px] 2xs:text-xs sm:text-sm text-neutral-400">{t('subtitle', 'Reward team members with your personal MCC')}</p>
        </div>
        <div className="flex gap-1.5 2xs:gap-2 shrink-0">
          <button
            onClick={() => setShowSendDialog(true)}
            className="px-2 2xs:px-3 py-1 2xs:py-1.5 bg-cyan-700 hover:bg-cyan-600 text-white rounded text-[11px] 2xs:text-xs sm:text-sm whitespace-nowrap"
          >
            {t('sendReward', 'Send Reward')}
          </button>
          <button
            onClick={() => setShowBatchDialog(true)}
            className="px-2 2xs:px-3 py-1 2xs:py-1.5 border border-neutral-700 text-neutral-400 hover:bg-neutral-800 hover:text-white rounded text-[11px] 2xs:text-xs sm:text-sm whitespace-nowrap"
          >
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 2xs:gap-3 sm:gap-4">
          <TerminalCard>
            <div className="text-[9px] 2xs:text-[10px] sm:text-xs text-[#5EEAD4] tracking-widest uppercase mb-1 truncate">{t('mccReceived', 'MCC Received')}</div>
            <div className="text-lg 2xs:text-xl xs:text-2xl sm:text-3xl font-bold text-white truncate">{Number(stats.total_received || 0).toFixed(2)}</div>
          </TerminalCard>
          <TerminalCard>
            <div className="text-[9px] 2xs:text-[10px] sm:text-xs text-[#5EEAD4] tracking-widest uppercase mb-1 truncate">{t('mccSent', 'MCC Sent')}</div>
            <div className="text-lg 2xs:text-xl xs:text-2xl sm:text-3xl font-bold text-cyan-400 truncate">{Number(stats.total_sent || 0).toFixed(2)}</div>
          </TerminalCard>
          <TerminalCard>
            <div className="text-[9px] 2xs:text-[10px] sm:text-xs text-[#5EEAD4] tracking-widest uppercase mb-1 truncate">{t('receiveCount', 'Times Received')}</div>
            <div className="text-lg 2xs:text-xl xs:text-2xl sm:text-3xl font-bold text-white truncate">{stats.receive_count}</div>
          </TerminalCard>
          <TerminalCard>
            <div className="text-[9px] 2xs:text-[10px] sm:text-xs text-[#5EEAD4] tracking-widest uppercase mb-1 truncate">{t('sendCount', 'Times Sent')}</div>
            <div className="text-lg 2xs:text-xl xs:text-2xl sm:text-3xl font-bold text-white truncate">{stats.send_count}</div>
          </TerminalCard>
        </div>
      )}
      ...
      <div className="flex gap-2 border-b border-neutral-800">
        <button
          ...
          className={`px-2 2xs:px-3 sm:px-4 py-1.5 2xs:py-2 text-[11px] 2xs:text-xs sm:text-sm whitespace-nowrap ${
        ...
        <TerminalCard title={t('rewardHistory', 'Reward History')}>
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[11px] 2xs:text-xs sm:text-sm">
                <thead>
                  <tr className="text-[10px] 2xs:text-xs text-neutral-400 border-b border-neutral-800">
                    <th className="text-left py-2 hidden sm:table-cell">{t('time', 'Time')}</th>
                    <th className="text-left py-2 hidden xs:table-cell">{t('sender', 'Sender')}</th>
                    <th className="text-left py-2">{t('recipient', 'Recipient')}</th>
                    <th className="text-right py-2">{t('amount', 'Amount')}</th>
                    <th className="text-left py-2 hidden md:table-cell">{t('reason', 'Reason')}</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map(record => (
                    <tr key={record.id} className="border-b border-neutral-800">
                      <td className="py-2 text-[10px] 2xs:text-xs text-neutral-500 hidden sm:table-cell">{formatDate(record.created_at)}</td>
                      <td className="py-2 text-neutral-300 truncate max-w-[120px] hidden xs:table-cell">{record.manager_email || record.manager_id}</td>
                      <td className="py-2 text-neutral-300 truncate max-w-[140px]">{record.recipient_email || record.recipient_id}</td>
                      <td className="py-2 text-right text-white whitespace-nowrap">+{Number(record.amount || 0).toFixed(2)} MCC</td>
                      <td className="py-2 text-[10px] 2xs:text-xs text-neutral-500 hidden md:table-cell">{record.reason || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === 'leaderboard' && (
        <div className="grid md:grid-cols-2 gap-3 2xs:gap-4 sm:gap-6">
              ...
              <div className="space-y-3">
                {topRecipients.map((entry, idx) => (
                  <div key={entry.user_id} className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 2xs:gap-3 min-w-0 flex-1">
                      <span
                        className={`w-5 h-5 2xs:w-6 2xs:h-6 flex items-center justify-center rounded text-[10px] 2xs:text-xs shrink-0 ${
                          idx < 3 ? 'bg-cyan-400/20 text-cyan-300' : 'bg-neutral-800 text-neutral-400'
                        }`}
                      >
                        {idx + 1}
                      </span>
                      <span className="text-neutral-300 text-[11px] 2xs:text-xs sm:text-sm truncate">{entry.email || entry.user_id}</span>
                    </div>
                    <span className="text-white text-[11px] 2xs:text-xs sm:text-sm whitespace-nowrap shrink-0">{Number(entry.total_received || 0).toFixed(2)} MCC</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-3">
                {topSenders.map((entry, idx) => (
                  <div key={entry.user_id} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-cyan-400 text-[11px] 2xs:text-xs sm:text-sm whitespace-nowrap shrink-0">{Number(entry.total_sent || 0).toFixed(2)} MCC</span>
                  </div>
                ))}
              </div>
            )}
