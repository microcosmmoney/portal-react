...
export function MicrocosmMiningPage({ basePath = '', onNavigate }: MicrocosmMiningPageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 py-3 space-y-2 2xs:px-3 2xs:py-4 2xs:space-y-3 sm:px-6 sm:py-6 sm:space-y-6">
      ...
      {}
      <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-3 2xs:p-4 sm:p-5 md:p-6 blockchain-card">
        <div className="space-y-3 2xs:space-y-4 mb-3 2xs:mb-4 md:mb-6">
          <div className="flex items-center flex-wrap gap-2">
            <span className="text-[10px] 2xs:text-xs bg-white/20 text-white px-2 py-0.5 rounded font-medium whitespace-nowrap">{t('x402Protocol', 'X402 Protocol')}</span>
            <span className="text-[10px] 2xs:text-xs bg-cyan-400/20 text-cyan-400 px-2 py-0.5 rounded font-medium whitespace-nowrap">Solana Mainnet</span>
          </div>

          <div>
            <div className="text-[10px] 2xs:text-xs text-[#5EEAD4] tracking-widest uppercase mb-1.5">MINING_PRICE</div>
            <div className="flex items-baseline gap-2 flex-wrap">
              <span className="text-lg 2xs:text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold text-cyan-400 font-mono whitespace-nowrap">
                1 MCC = {miningPrice > 0 ? fmt(miningPrice, 4) : '--'} USD
              </span>
              <span className="text-[9px] 2xs:text-[10px] bg-cyan-400/20 text-cyan-400 px-1.5 py-0.5 rounded whitespace-nowrap">
                base × 4
              </span>
            </div>
          </div>

          <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-[10px] 2xs:text-xs text-neutral-500">
          ...
          <button
            onClick={handleMine}
            className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 sm:py-4 text-sm sm:text-base font-medium bg-cyan-700 hover:bg-cyan-600 text-white rounded-md whitespace-nowrap transition-colors"
          >
            <IconZap className="w-4 h-4 sm:w-5 sm:h-5" />
            {t('startMinting', 'Start Minting')}
          </button>
        </div>
