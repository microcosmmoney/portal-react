...
export function MicrocosmWalletPage({ basePath = '', onNavigate }: MicrocosmWalletPageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 py-3 space-y-2 2xs:px-3 2xs:py-4 2xs:space-y-3 xs:px-4 xs:space-y-4 sm:px-6 sm:py-6 sm:space-y-6">
      ...
      {}
      <div className="bg-neutral-900 border border-neutral-700 rounded-lg hover:border-cyan-400/50 transition-colors">
        <div className="p-3 2xs:p-4 sm:p-5 md:p-6">
          <div className="flex items-start gap-3 mb-3">
            <div className="p-2 2xs:p-3 bg-cyan-400/20 rounded-xl border border-cyan-400/30 shrink-0">
              <IconWallet className="w-5 h-5 2xs:w-6 2xs:h-6 sm:w-8 sm:h-8 text-cyan-400" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-[10px] 2xs:text-xs sm:text-sm text-neutral-400 tracking-wider mb-1">{t('mccBalance', 'MCC Balance')}</div>
              <div className="text-xl 2xs:text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold text-white font-mono truncate">
                {mccLoading ? <Spinner /> : mask(fmt(mccBalance))}
              </div>
              <div className="text-[10px] 2xs:text-xs sm:text-sm text-neutral-500 mt-1 truncate">
                {mccLoading ? '' : mask(`${fmtUSD(totalUsd)}`)}
                {!mccLoading && price > 0 && (
                  <span className="ml-2 text-[10px] 2xs:text-xs text-neutral-600">
                    @ {fmtUSD(price)}/MCC
                  </span>
                )}
              </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2 sm:py-2.5 bg-cyan-700 hover:bg-cyan-600 text-white rounded text-xs 2xs:text-sm transition-colors whitespace-nowrap"
          >
