...
export function MicrocosmOrganizationPage({ basePath = '', onNavigate }: MicrocosmOrganizationPageProps) {
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto px-3 py-4 space-y-3 xs:px-4 xs:space-y-4 sm:px-6 sm:py-6 sm:space-y-6">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-white tracking-wider">Organization</h1>
          <p className="text-xs sm:text-sm text-neutral-400 mt-1">Ecosystem structure & territory management</p>
        </div>
        <div className="flex items-center justify-center py-20">
          <Spinner />
          <span className="ml-3 text-neutral-400 text-sm">Loading organization data...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 py-4 space-y-3 xs:px-4 xs:space-y-4 sm:px-6 sm:py-6 sm:space-y-6">
      ...
      {}
      <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-3 sm:p-4 md:p-6 hover:border-cyan-400/50 transition-colors">
        <div className="mb-2 sm:mb-3 md:mb-4 flex items-center gap-2 sm:gap-3">
          <IconUsers size={20} className="text-cyan-400" />
          <div>
            <div className="text-xs sm:text-sm font-bold text-cyan-400 tracking-wide">User Statistics</div>
            <div className="text-[9px] sm:text-[10px] text-neutral-500 font-mono tracking-wider">User Statistics</div>
          </div>
        </div>
        <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-3">
          {[
            { label: 'Total Users', value: totalUsers, icon: <IconUsers size={16} className="text-cyan-400/70" /> },
            { label: 'Miner', value: miners, icon: <IconPickaxe size={16} className="text-cyan-400/70" /> },
            { label: 'Commander', value: commanders, icon: <IconUserCheck size={16} className="text-cyan-400/70" /> },
            { label: 'Pioneer', value: pioneers, icon: <IconCompass size={16} className="text-cyan-400/70" /> },
            { label: 'Unassigned', value: unassigned, icon: <IconUserX size={16} className="text-cyan-400/70" /> },
          ].map(({ label, value, icon }) => (
            <div key={label} className="p-3 bg-neutral-800 rounded border border-neutral-700">
              <div className="flex items-center gap-1.5 mb-1">
                {icon}
                <span className="text-xs text-cyan-400/70 tracking-wider">{label}</span>
              </div>
              <div className="text-lg font-bold text-white font-mono">{fmt(value)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4">
          ...
          front={
            <>
              <div className="mb-4 flex items-center gap-3">
                <IconNetwork size={20} className="text-cyan-400" />
                <div>
                  <div className="text-sm font-bold text-cyan-400 tracking-wide">Territory Distribution</div>
                  <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Territory Distribution</div>
                </div>
              </div>
              <div className="grid grid-cols-4 gap-3">
                {[
                  { label: 'Station', count: stationCount, icon: <IconBuilding size={20} className="text-cyan-400/70 mx-auto mb-2" /> },
                  { label: 'Matrix', count: matrixCount, icon: <IconGrid size={20} className="text-cyan-400/70 mx-auto mb-2" /> },
                  { label: 'Sector', count: sectorCount, icon: <IconMap size={20} className="text-cyan-400/70 mx-auto mb-2" /> },
                  { label: 'System', count: systemCount, icon: <IconGlobe size={20} className="text-cyan-400/70 mx-auto mb-2" /> },
                ].map(({ label, count, icon }) => (
                  <div key={label} className="text-center p-3 bg-neutral-800 rounded border border-neutral-700">
                    {icon}
                    <div className="text-lg font-bold text-white font-mono">{count}</div>
                    <div className="text-xs text-cyan-400/70 tracking-wider">{label}</div>
                  </div>
                ))}
              </div>
          ...
          back={
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <IconNetwork size={20} className="text-cyan-400" />
                <div>
                  <div className="text-sm font-bold text-cyan-400 tracking-wide">Territory System</div>
                  <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Territory System</div>
                </div>
              </div>
        ...
        <FlipCard
          front={
            <>
              <div className="mb-4 flex items-center gap-3">
                <IconBuilding size={20} className="text-cyan-400" />
                <div>
                  <div className="text-sm font-bold text-cyan-400 tracking-wide">Vault Balance</div>
                  <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Vault Balance</div>
                </div>
              </div>
              <div className="p-4 bg-neutral-800 rounded border border-neutral-700">
                <div className="text-3xl font-bold text-cyan-300 font-mono">{formatMCD(totalVaultMCD)}</div>
                <div className="text-sm text-cyan-400/50 mt-1">MCD</div>
              </div>
              <div className="mt-3 text-xs text-neutral-500">
          ...
          back={
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <IconBuilding size={20} className="text-cyan-400" />
                <div>
                  <div className="text-sm font-bold text-cyan-400 tracking-wide">Vault Mechanism</div>
                  <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Vault Mechanism</div>
                </div>
              </div>
              <p className="text-sm text-neutral-300 leading-relaxed">
              ...
              <div className="text-sm text-neutral-400 leading-relaxed space-y-1.5">
                ...
                <p><span className="text-cyan-400/80">Transparency</span> — All vault balances and distributions are on-chain verifiable</p>
              </div>
              <p className="text-sm text-neutral-400 leading-relaxed">
      ...
      <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-6 hover:border-cyan-400/50 transition-colors">
        <div className="mb-4 flex items-center gap-3">
          <IconCrown size={20} className="text-cyan-400" />
          <div>
            <div className="text-sm font-bold text-cyan-400 tracking-wide">Level Progression</div>
            <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Level Progression</div>
          </div>
        </div>
        <div className="grid grid-cols-2 xs:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
          {[
            ...
            <div key={level} className="flex items-center gap-3 p-3 bg-neutral-800 rounded border border-neutral-700">
              <div className="text-2xl font-bold text-cyan-400 font-mono leading-none">{level}</div>
              <div>
                <div className="text-sm font-medium text-neutral-200">{en}</div>
                <div className="text-xs text-neutral-400">{zh}</div>
                <div className="text-xs text-neutral-500">{condition}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-6 hover:border-cyan-400/50 transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <IconLayers size={20} className="text-cyan-400" />
            <div>
              <div className="text-sm font-bold text-cyan-400 tracking-wide">Territory Architecture</div>
              <div className="text-[10px] text-neutral-500 font-mono tracking-wider">Territory Structure</div>
            </div>
          </div>
        ...
        {}
        <div className="relative rounded-lg border border-neutral-800 overflow-hidden bg-neutral-950/50 p-6">
          {}
          ...
          {}
          <div className="relative z-10 mt-4 sm:mt-6 grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              <div key={type} className={`rounded-lg border p-3 ${color}`}>
                <div className={`text-sm font-bold font-mono ${textColor}`}>{type}</div>
                <div className="text-xs text-neutral-400 mt-0.5">{desc}</div>
                <div className="flex items-baseline justify-between mt-2">
                  <span className="text-[10px] text-neutral-500 font-mono">{capacity} users</span>
                  <span className={`text-lg font-bold font-mono ${textColor}`}>{count}</span>
                </div>
              </div>
            ))}
          </div>
