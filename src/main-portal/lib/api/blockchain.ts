...
export async function getMcdAccountStatus(
  uid: string
): Promise<McdAccountStatus | null> {
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/account-status/${uid}`,
      { method: 'GET' }
    )

    if (response.success && response.data) {
      return response.data as McdAccountStatus
    }
    return null
  } catch (error) {
...
export async function ensureUserMcdAccount(
  uid: string
): Promise<McdAccountResult> {
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/account/ensure/${uid}`,
      { method: 'POST' }
    )

    if (response.success && response.data) {
      return response.data as McdAccountResult
    }
...
export async function batchGetMcdAccountStatus(
  uids: string[]
): Promise<Record<string, McdAccountStatus>> {
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/batch-status`,
      {
        method: 'POST',
    ...
    if (response.success && response.data) {
      return response.data as Record<string, McdAccountStatus>
    }
    return {}
  } catch (error) {
...
export async function getZombieAccounts(
  options: {
    limit?: number
    offset?: number
    min_inactive_days?: number
  ...
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/zombie-accounts?limit=${limit}&offset=${offset}&inactive_days=${min_inactive_days}`,
      { method: 'GET' }
    )

    if (response.success && response.data) {
      return response.data as ZombieAccountsResult
    }
...
export async function closeMcdAccount(
  uid: string,
  reason: 'zombie' | 'user_request' | 'admin' = 'zombie'
): Promise<ClosureResult> {
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/close-account/${uid}`,
      {
        method: 'POST',
...
export async function batchCloseMcdAccounts(
  uids: string[],
  reason: 'zombie' | 'admin' = 'zombie'
): Promise<BatchClosureResult> {
  try {
    const response = await fetchApi(
      `/blockchain/mcd/admin/batch-close-accounts`,
      {
        method: 'POST',
    ...
    if (response.success && response.data) {
      return response.data as BatchClosureResult
    }
...
export async function getRentStatistics(
  ...
  try {
    ...
    if (response.success && response.data) {
      return response.data as RentStatistics
    }
...
export async function getClosureHistory(
  ...
  try {
    ...
    if (response.success && response.data) {
      return response.data as ClosureHistory
    }
...
export async function getSystemAddresses(
...
): Promise<{ addresses: SystemAddress[]; total: number }> {
  ...
  try {
    ...
    const response = await fetchApi(`/blockchain/system-addresses/admin/list?${params.toString()}`, { method: 'GET' })
    if (response.success && response.data) {
      return response.data as { addresses: SystemAddress[]; total: number }
    }
...
export async function getSystemAddressStats(): Promise<SystemAddressStats> {
  try {
    const response = await fetchApi('/blockchain/system-addresses/admin/stats', { method: 'GET' })
    if (response.success && response.data) {
      return response.data as SystemAddressStats
    }
...
export async function updateSystemAddress(
  id: number,
  data: { description?: string; status?: string; archived_reason?: string }
): Promise<{ success: boolean; id?: number; name?: string; status?: string; error?: string }> {
  try {
    const response = await fetchApi(`/blockchain/system-addresses/admin/${id}`, {
      method: 'PUT',
...
export async function seedSystemAddresses(): Promise<{ success: boolean; inserted: number; updated: number; total: number }> {
  try {
    const response = await fetchApi('/blockchain/system-addresses/admin/seed', { method: 'POST' })
    return response
  } catch (error) {
